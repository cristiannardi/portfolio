...
const imcCalc = () => {
    if(!peso.value || !altura.value || !nome.value) return resultado.textContent = 'Preencha todos os dados'
    const valorImc = (+peso.value/(+altura.value * +altura.value)).toFixed(1)
    resultado.textContent = resultado.textContent = `${nome.value} - ${getText(valorImc)}`;
}
